...
export async function LiveResults() {
  ...
  const tickerRow = (hidden: boolean) => (
    <ul
      ...
      className="flex w-max shrink-0 items-center gap-10 whitespace-nowrap px-5 py-2.5 text-sm text-dt-dim"
    >
  ...
  return (
    <section className="mx-auto max-w-6xl px-4 py-16 md:py-24">
      <Reveal className="mx-auto max-w-2xl text-center">
        ...
        <p className="mt-3 text-dt-dim">{t("home.liveSubtitle")}</p>
      </Reveal>

      {/* LED-board ticker */}
      <Reveal delay={0.05}>
        <div className="mt-10 flex items-stretch overflow-hidden rounded-lg border border-dt-line bg-dt-card">
      ...
      {results.length === 0 ? (
        <Reveal delay={0.1}>
          <p className="mt-8 rounded-lg border border-dashed border-dt-line p-6 text-center text-sm text-dt-dim">
            {t("home.liveEmpty")}
          </p>
        </Reveal>
      ) : (
            ...
            <div className="mt-8 grid gap-4 md:grid-cols-2">
              {live.map((m, i) => (
                <Reveal key={m.id} delay={0.1 + i * 0.05} className="h-full">
                  <article className="h-full rounded-lg border border-dt-line bg-dt-card p-5 shadow-low">
                    <div className="flex items-center justify-between gap-2">
                      <Link
                        href={`/turfs/${m.turfSlug}`}
                        className="truncate text-sm font-medium hover:text-dt-green"
                      >
                        {m.turfName}
                      </Link>
                    ...
                    <div className="mt-5 grid grid-cols-[1fr_auto_1fr] items-center gap-3">
                      ...
                      <p className="font-heading text-4xl font-bold tabular-nums">
                        {m.homeScore ?? 0}
                        <span className="mx-1.5 text-lg text-dt-dim">
                          :
                        </span>
                        {m.awayScore ?? 0}
                      </p>
                    ...
                    <div className="mt-5 flex items-center justify-between text-xs text-dt-dim">
              ...
              <ul className="divide-y divide-dt-line overflow-hidden rounded-lg border border-dt-line">
                {recent.map((m) => (
                  <li key={m.id}>
                    <Link
                      href={`/matches/${m.id}`}
                      className="flex items-center gap-3 bg-dt-card p-3 text-sm transition-colors hover:bg-dt-card2/40"
                    >
                      ...
                      <span className="min-w-0 flex-1 truncate">
                        {m.homeName ?? tbd}{" "}
                        <span className="text-dt-dim">vs</span>{" "}
                        {m.awayName ?? tbd}
                      </span>
                      <span className="hidden shrink-0 text-xs text-dt-dim sm:block">
                        {m.turfName}
                      </span>
