...
/**
 * Approved project loader (SS-loading). Single source of truth for the
 * spinner-type loading visual. Do not introduce alternative spinner designs.
 */
export function Loader({
  ...
  return (
    <svg
      {...(hidden ? { "aria-hidden": true as const } : { role: "status", "aria-label": label })}
      viewBox="0 0 866 866"
      xmlns="http://www.w3.org/2000/svg"
      width={size}
      height={size}
      className={cn("app-loader shrink-0", className)}
      {...props}
    >
      <svg
        id="app-loader-layer"
...
/**
 * Page-level loading state: the approved loader centered on a dark overlay.
 */
export function LoaderOverlay({ label = "Loading" }: { label?: string }) {
  return (
    ...
    >
      <Loader
        size={300}
        className="max-h-[60vw] max-w-[60vw]"
        label={label}
      />
